...
- **`provider_failover.py`** — Cross-provider failover on quota exhaustion. When the primary provider hits quota, switches to fallback providers configured via `fallback_providers` in `config.yaml`. Probes primary recovery every 5 minutes and switches back automatically
