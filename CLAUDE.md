...
- **Commit messages** follow [`docs/commit-conventions.md`](docs/commit-conventions.md): Use semantic prefixes (`feat`/`fix`/`refactor`/`docs`/`test`/`chore`), include case ID from mission context or `Case: N/A:`, write descriptive bodies, and **always** end with `Changelog:` footer (even if empty). Footer order: Co-Authored-By → Refs → Changelog (last).
